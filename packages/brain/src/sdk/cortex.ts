...
export class Cortex {
  ...
  constructor(private config: CortexConfig) {
    if (config.hosted) {
      ...
      const { _setDb } = require('@clude/shared/core/database');
      _setDb(this.db);

      // Inject Anthropic client if provided
      if (config.anthropic?.apiKey) {
        const client = new Anthropic({ apiKey: config.anthropic.apiKey });
        const { _setAnthropicClient } = require('@clude/shared/core/claude-client');
        _setAnthropicClient(client, config.anthropic.model);
      }

      // Inject embedding config if provided
      if (config.embedding) {
        const { _configureEmbeddings } = require('@clude/shared/core/embeddings');
  ...
  async init(): Promise<void> {
    ...
    if (this.config.encryption?.solanaSecretKey) {
      const { configureEncryption } = require('@clude/shared/core/encryption');
    ...
    if (this.config.solana?.memoryRegistryProgramId) {
      const { initializeRegistry } = require('@clude/shared/core/solana-client');
  ...
  async verifyOnChain(memoryId: number): Promise<boolean> {
    ...
    const { hydrateMemories } = require('../memory');
    const { verifyMemoryOnChain } = require('@clude/shared/core/solana-client');
