...
import { Cortex } from '../../packages/brain/src/sdk';
...
// When opts.readerModel is a GPT model, transparently routes through OpenAI
// while returning an Anthropic-shaped response. Judge/extraction calls
// (Sonnet/Haiku models) fall through to the real Anthropic client unchanged.

function isOpenAIModel(m: string): boolean {
  return m.startsWith('gpt-') || m.startsWith('o4-') || m.startsWith('o1-') || m.startsWith('o3-');
}

function wrapAsAnthropic(openaiResp: any): any {
  return {
    ...
    stop_reason: openaiResp.choices[0].finish_reason,
  };
}

let _realAnthropic: Anthropic;
let _openai: OpenAI;

// Drop-in shim: same interface as `anthropic.messages.create()`
const anthropic = {
  messages: {
    create: async (opts: any): Promise<any> => {
      if (!isOpenAIModel(opts.model)) {
        return _realAnthropic.messages.create(opts);
      }
      // Route through OpenAI with retry on 429
      for (let attempt = 0; attempt < 5; attempt++) {
        try {
          ...
          return wrapAsAnthropic(r);
        } catch (e: any) {
          const msg = String(e?.message || e);
          if ((msg.includes('429') || msg.includes('rate_limit')) && attempt < 4) {
            const wait = 30_000 * (2 ** attempt);
            console.error(`[openai-shim] rate-limited, waiting ${wait / 1000}s (attempt ${attempt + 1}/5)`);
      ...
      throw new Error('[openai-shim] retries exhausted');
    },
  },
};
...
- If the quotes don't clearly answer, check the original context too`,
    ...
    // Only fall through if it's a hard refusal at the start (not just hedging mid-answer)
    const ssaHardIdk = /^(i don't have|i do not have|i cannot find|there is no|no information|not mentioned)/i.test(ssaAnswer);
    if (!ssaHardIdk) {
      return ssaAnswer;
    }
...
- If the question asks about something the user NEVER did (and no evidence exists), it is correct to say "not enough information" — do not fabricate an answer`,
    ...
    // Only fall through on hard refusal — keep answers that have any concrete content
    const multiHardIdk = /^(i don't have|i do not have|i cannot find|there is no|no information provided is not enough)/i.test(multiAnswer);
    if (!multiHardIdk) {
      return multiAnswer;
    }
    // Fall through to standard single-pass approach below
...
MANDATORY PROCESS — follow these steps exactly:
1. Find the relevant event(s) in the TIMELINE by name (use semantic matching: "baking class" = "culinary workshop", "guitar repair" = "guitar shop")
2. Extract the EXACT date(s) from the timeline entry
3. For calculation questions: write the arithmetic explicitly, e.g. "Event on 2023/05/15, question date 2023/07/20, difference = 66 days"
4. State the final answer at the end, concisely

Rules:
- For "how many days ago": [event date] → [question date ${questionDate || 'today'}] = N days. Show the subtraction.
- For "how many days between X and Y": [date1] → [date2] = N days. Show the subtraction.
- For "how many weeks/months": convert days to weeks or months explicitly
- For ordering: list each event with its YYYY/MM/DD date, then name which came first/last
- If an event is NOT in the timeline, search the ORIGINAL CONTEXT for it — the timeline may have missed it
- NEVER say "I don't have information" — the date IS in the context, keep searching
- Final answer should be the specific number or event name, stated plainly`,
      messages: [{
        role: 'user',
    ...
    // Only fall through on a hard refusal at the start of the answer
    const temporalHardIdk = /^(i don't have|i do not have|i cannot find|there is no|no information|not mentioned|not found in)/i.test(temporalAnswer);
    if (!temporalHardIdk) {
...
- Answer in 1-10 words maximum — be extremely concise`
      : `Return ONLY the LATEST/MOST RECENT value. This is a knowledge-update question — IGNORE older values.

MANDATORY RULES:
1. Look at the VERSIONS list. Find the entry with the LATEST/HIGHEST date.
2. That entry's value IS your answer. Do NOT use any earlier entry.
3. Answer in 1-10 words — extremely concise. Example: "42", "New York", "3 times a week", "cancelled".
4. Do NOT say "as of [date]" or add any date hedging — just state the current value directly.
5. Do NOT return an older value even if it seems "more certain" — the latest date wins, always.
6. If the ENTITY CHECK notes a mismatch (e.g., question asks about "Dr. Smith" but memories mention "Dr. Johnson"), say "The information provided is not enough to answer about the specific entity asked about."
7. For counting (how many X do I have now): sum ALL acquisitions minus any disposals up to the latest date.
8. If versions list shows "LATEST VALUE: X", your answer IS X. State it directly.`;
...
- NEVER say "I don't have information"
- NEVER hedge or add qualifiers — just the current value
- If version history says "LATEST VALUE: X", answer "X" directly`,
      messages: [{
        role: 'user',
        content: `Version history:\n${versions}\n\nOriginal memory context:\n${context}\n\nQuestion: ${question}\n\nCurrent value (concise answer):`,
      }],
    });

    let kuAnswer = stage2.content[0].type === 'text' ? stage2.content[0].text.trim() : '';

    // Only fall through on hard refusal — keep most KU two-stage answers
    const kuHardIdk = /^(i don't have|i do not have|i cannot find|there is no|no information|not mentioned)/i.test(kuAnswer);
    if (!kuHardIdk) {
      return kuAnswer;
    }
    // Fall through to standard single-pass approach below
...
`);
  ...
  _realAnthropic = new Anthropic({ apiKey: ANTHROPIC_KEY });
  _openai = new OpenAI();
