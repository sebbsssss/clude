...
**Re-delegate (client-driven — server has no DEK):** dashboard unwraps the owner DEK locally → re-wraps to provider pubkey → posts it. Server re-inserts the provider row, decrypts the sealed `summary`/`embedding`, restores their plaintext columns (restoring `summary` auto-repopulates the generated `ts_summary`), rebuilds `content_tokens` via the `set_memory_content_tokens` RPC from the decrypted content, sets `provider_delegated = true`. **Validate-by-decrypt (fixes L10):** before accepting, the server unwraps the posted wrap with the provider private key → candidate DEK, then decrypts the memory's stored content ciphertext with it. The `secretbox` Poly1305 tag authenticates only when the wrap carries the correct per-memory DEK, so a successful decrypt proves the wrap is genuine; a wrap of any other key fails the decrypt → reject (422). This **supersedes the earlier nonce challenge/response idea**: because the server already holds the ciphertext, it can prove the wrap yields a working DEK directly — no extra round-trip and no server-side nonce state to keep.
...
- **Revoke/re-delegate tests**: revoke deletes provider wrap + seals summary/embedding + clears `summary` (asserting `ts_summary` auto-empties) + sets `content_tokens = NULL` + sets flag + evicts cached DEK; re-delegate restores summary (asserting `ts_summary` repopulates) + rebuilds `content_tokens` + validates-by-decrypt (a forged wrap of a different key is rejected, RPC never called); server can't decrypt post-revoke.
