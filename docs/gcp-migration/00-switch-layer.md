...
- **Slice 3: the Vertex embedding shadow space (gated).**
  - Add a `vertex` provider to `embeddings.ts`. Vertex does not fit the current static-key/OpenAI-shape `PROVIDERS` map: it uses the aiplatform `:predict` endpoint, a GCP OAuth access token (service account, not a static key), body `{instances:[{content}],parameters:{outputDimensionality:1024}}`, response `{predictions:[{embeddings:{values}}]}`. So the provider abstraction grows an optional per-provider body builder, response parser, and async auth.
  - Target `gemini-embedding-001` truncated to 1024 dims (MRL) so `vector(1024)` columns, HNSW, and the `match_*` RPCs are unchanged.
  - Migration: add shadow columns `embedding_vertex vector(1024)` (+ per-fragment) and a shadow HNSW index. Backfill script re-embeds the corpus into the shadow column.
  - Recall targets the column chosen by `activeEmbeddingSpace()` (a `match_*_vertex` RPC variant, or a column parameter). Reranker stays Voyage for now.
