...
- **Slice 2: wire the spine into the hot paths. DONE.**
  ...
  - `bootstrap.ts` — the four timer-start blocks (recall canary, C2 memory outbox drain, marketplace delivery poller, title reconciliation) are wrapped in `if (shouldRunInProcessTimers())`, with an explicit log on the disabled branch. This is the fix for the single-instance trap: on Cloud Run the server sets `RUN_INPROCESS_TIMERS=false` and exactly one worker owns the timers. NOTE for the cutover slice: whichever process runs with `RUN_INPROCESS_TIMERS=true` must also carry `MEMORY_OUTBOX` (plus DB creds) or C2 enrichment silently stops draining.
